...
-   `gerry_26_cat` is a character variable, so the plot orders its values alphabetically
-   `fct_relevel()` creates a factor with levels in the specified order
-   We can use `mutate()` to overwrite `gerry_26_cat` and store the change
...
Use `if_else()` to recode a logical (true/false) condition:

```{r}
gerrymander_24 |>
  mutate(
    trump_or_harris = if_else(
      trump_24 >= 50,  # condition
      "Trump",  # action if true
      "Harris",  # action if false
    )
  ) |>
  select(district, trump_24, trump_or_harris)
```
...
::: task
How would you write the following pipeline with `count()` instead? (Select all that apply)
...
::: wooclap-choices
A) `gerrymander_24 |> arrange(state) |> count()`
B) `gerrymander_24 |> count(state) |> arrange(desc(n))`
C) `gerrymander_24 |> count(state) |> sort(n)`
D) `gerrymander_24 |> count(state, sort = TRUE)`
:::
